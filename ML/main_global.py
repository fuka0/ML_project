...
sss_tl = StratifiedShuffleSplit(n_splits=4, random_state=22, test_size=0.3) # cross validation for Transfer model
...
for target_subject in target_subjects:
    ...
    for fold, (train_idxs, test_idxs) in enumerate(sgkf.split(X, y, groups=subject_ids)):
        ...
        #                 CSVLogger(log_dir / f"global_{target_subject}_model.csv", append=False)]
        callbacks_list = [
                        ModelCheckpoint(f"{model_dir}/target_{target_subject}_model.keras", save_best_only=True, monitor='val_loss', mode='min'),
                        CSVLogger(log_dir / f"global_{target_subject}_model.csv", append=False)]

        model.fit(X_train_combined, y_train_combined, epochs=100, batch_size=30, validation_data=(X_test, y_test), callbacks=callbacks_list)
...
# //////////Transfer Learning part//////////
        ...
        for train, test in sss_tl.split(X_sstl, y_sstl):
            ...
            X_train_sstl, X_val, y_train_sstl, y_val = train_test_split(X_train_sstl, y_train_sstl, test_size=0.3, random_state=22, stratify=y_train_sstl)
            ...
            #                     CSVLogger(log_dir / f"target_{target_subject}_model.csv", append=False)]
            sstl_callbacks_list = [CSVLogger(log_dir / f"target_{target_subject}_model.csv", append=False)]

            # Transfer Learning
            history_sstl = general_model.fit(X_train_sstl, y_train_sstl, epochs=50, batch_size=18, validation_data=(X_val, y_val), callbacks=sstl_callbacks_list)
